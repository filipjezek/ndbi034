...
def twlcs(file1: MidiFile, file2: MidiFile) -> int:
    if len(file1.tracks) == 0 or len(file2.tracks) == 0:
        return 0
    track1, track2 = filter_track(file1.tracks[0], 20), filter_track(file2.tracks[0]) # both files should have only one track
    a, b = len(track1), len(track2)
    if a == 0 or b == 0:
        return 0
    matrix = np.zeros((a + 1, b + 1), np.int32)
    for i in range(1, a + 1):
        for j in range(1, b + 1):
            if track1[i - 1] == track2[j - 1]:
